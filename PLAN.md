...
| D11 | Entry surface + dispatch runtime | In-Agent `/orca dispatch` is primary; tmux + multi-pane on-demand. CLI `orca` preserved. Dispatch resolves per call to M1 (workflow-only) / M2 (cc Task subagent) / M3 (tmux pane worker). | Direction: [`docs/research/competitor-analysis/orca-evolution-proposal.md`](docs/research/competitor-analysis/orca-evolution-proposal.md). Mechanics: [`docs/design/dispatch-runtime.md`](docs/design/dispatch-runtime.md). |
