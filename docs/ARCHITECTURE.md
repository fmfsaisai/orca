...
- `tmux-bridge read coder 100` 比 `tmux capture-pane -t orca:main.1 -p -J -S -100` 省 token
- Read Guard 机制防止盲操作
...
- Lead 和 Coder 加载同一个 `skills/orca/SKILL.md`
- 角色由激活命令决定（`/orca` = lead，`$orca` = coder），无需环境变量区分
- 条件激活：`$ORCA` 环境变量 + SessionStart hook，非 orca 环境自动忽略，新会话自动激活
...
等 openai/codex#10390 修复后可切回 socket 白名单方案。
